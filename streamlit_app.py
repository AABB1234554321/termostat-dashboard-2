...
def get_reward(state, action):
    if abs(state - get_state(thermostat_setting)) <= 1:
        return 10
    elif action == 1 and state > get_state(thermostat_setting + 0.5):
        return -10
    elif action == 0 and state < get_state(thermostat_setting - 0.5):
        return -5
    else:
        return 0
...
if st.button("Run Simulation"):
  ...
  plt.plot([time_q[0], time_q[-1]]), [outside_temperature, outside_temperature], label="Outside Temperature", color="gray")
    ...
    plt.ylabel("Temperature (°C) / Heater Output (%)")  # Updated y-axis label
    plt.title("Thermostat Simulation: Q-Learning vs. PID")
    plt.legend()
    plt.grid(axis='y')

    # Shade the loss area only for Q-Learning if it's selected
    if simulation_type == "Q-Learning" or simulation_type == "Both":
    ...
    # Shade the loss area only for PID if it's selected
    if simulation_type == "PID" or simulation_type == "Both":
